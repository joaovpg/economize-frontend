...
function PrivateLayout() {
  ...
  return (
    <div className="app-background relative isolate min-h-svh overflow-hidden bg-canvas text-foreground selection:bg-brand-soft selection:text-brand-hover">
      <div className="relative z-1 mx-auto min-h-svh w-full max-w-310 border-x border-[color-mix(in_oklch,var(--color-border)_72%,transparent)] bg-[color-mix(in_oklch,var(--color-canvas)_34%,transparent)] max-[48rem]:border-x-0">
        <header className="flex items-center justify-between gap-4.5 border-b border-border bg-[color-mix(in_oklch,var(--color-canvas)_76%,transparent)] px-6 py-4.5 backdrop-blur-[14px] max-[60rem]:px-5 max-[48rem]:grid max-[48rem]:grid-cols-[minmax(0,1fr)_auto] max-[48rem]:gap-3.5 max-[48rem]:p-4">
          <Link
            className="gap-2.5 text-title-compact tracking-tight whitespace-nowrap text-foreground! no-underline"
            preload="intent"
            to="/summary"
          ...
          <nav
            className="flex items-center justify-center gap-1.5 rounded-full border border-border bg-[color-mix(in_oklch,var(--color-surface)_58%,transparent)] p-1 max-[48rem]:col-span-2 max-[48rem]:w-full max-[48rem]:justify-start max-[48rem]:overflow-x-auto"
          ...
          <Link
            className="gap-2 text-button whitespace-nowrap text-brand! no-underline data-hovered:text-brand-hover! data-hovered:underline data-hovered:underline-offset-4 max-[60rem]:hidden"
            preload="intent"
            to="/profile"
